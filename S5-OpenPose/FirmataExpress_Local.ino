...
static const int NEOPIXEL_HANDLE_REGISTER =              0x66;
static const int NEOPIXEL_HANDLE_COLOR =              0x67;
static const int NEOPIXEL_HANDLE_REGISTER_RESPONSE =   0x53;
static const int NEOPIXEL_HANDLE_COLOR_RESPONSE =      0x54;
/*==============================================================================
   GLOBAL VARIABLES
  ============================================================================*/
...
//Adafruit_NeoPixel *neopixels = NULL;//new Adafruit_NeoPixel(10, 7, NEO_GRBW + NEO_KHZ800);
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  switch (command) {
    ...
    case SONAR_CONFIG :
      ...
      break ;
    case NEOPIXEL_HANDLE_REGISTER:    
      //Firmata.sendString("NEOPIXEL_HANDLE_REGISTER received");
      /*int local_pin = argv[0];
      int led_count = argv[1];
      
      if (neopixels != NULL) {
        delete neopixels;
      }
      neopixels = new Adafruit_NeoPixel(led_count, local_pin, NEO_GRBW + NEO_KHZ800);
      neopixels->begin();
      neopixels->show();
      */
      Serial.print("NOOOOOO");
      Firmata.write(START_SYSEX);
      Firmata.write((byte)STRING_DATA);
      Firmata.write((byte)argv[0]);
      Firmata.write((byte)argv[1]);
      Firmata.write(END_SYSEX);
      systemResetCallback();
      break;
    case NEOPIXEL_HANDLE_COLOR:
      int led_count = argv[0];
      int red = argv[1];
      int green = argv[2];
      int blue = argv[3];
      ...
      Firmata.write((byte)argv[0]);
      Firmata.write((byte)argv[1]);
      //Firmata.write((byte)argv[2]);
      //Firmata.write((byte)argv[3]);
      Firmata.write(END_SYSEX);*/
      break;
    
     /* Firmata.sendString("NEOPIXEL_HANDLE_COLOR received");
      int index = argv[0];
      int red = argv[1];
      int green = argv[2];
      int blue = argv[3];
      ...
      {
        neopixels->setPixelColor(i, neopixels->Color(red, green, blue));
      }
      neopixels->show();
      */
      break;
